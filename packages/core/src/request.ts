...
export class Request {
  ...
  constructor(
    ...
    const { href, origin, protocol, host, hostname, path, querystring, query } =
      parseRequestURL(req.url!, getOrigin(req, behindProxy));
    this.#href = href;
    this.#origin = origin;
  ...
  is(...values: readonly string[]): string | false {
    if (this.hasBody) {
      return ContentType.get(this.#headers)?.is(...values) ?? false;
    }
    return false;
  }
  ...
  negotiateType(...values: readonly string[]): string | null {
    return this.#getAccept().negotiate(...values);
  }
  ...
  negotiateEncoding(...values: readonly string[]): string | null {
    return this.#getAcceptEncoding().negotiate(...values);
  }
  ...
  negotiateLanguage(...values: readonly string[]): string | null {
    return this.#getAcceptLanguage().negotiate(...values);
  }
