...
export class AcceptEncoding implements Header {
  ...
  static identity(): AcceptEncoding {
    return new AcceptEncoding("identity");
  }

  private readonly _list: AcceptedEncoding[] = [];
  ...
  add(value: string, q = 1): this {
    this._list.push(new AcceptedEncoding(value, q));
    return this;
  }
  ...
  negotiateAll(...candidates: readonly string[]): string[] {
    ...
    return negotiateAll(candidates, this._list, (v) => v.toLowerCase());
  }
