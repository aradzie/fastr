...
export class AcceptLanguage implements Header {
  ...
  static get(headers: GetHeader): AcceptLanguage | null {
    return getHeader(AcceptLanguage, headers);
  }

  static tryGet(headers: GetHeader): AcceptLanguage | null {
    return tryGetHeader(AcceptLanguage, headers);
  }
  ...
  readonly #list: AcceptedLanguage[] = [];
  ...
  add(value: string, q = 1): this {
    this.#list.push(new AcceptedLanguage(new Language(value), q));
    return this;
  }
  ...
  negotiateAll(...candidates: readonly string[]): string[] {
    ...
    return negotiateAll(candidates, this.#list, (v) => new Language(v));
  }
